...
export class HealthScoreService {
  ...
  async calculateParcelHealthScore(parcelId: string, farmerId: string) {
    ...
    if (parcel.soilPh !== null) {
      if (parcel.soilPh >= 6.0 && parcel.soilPh <= 7.5) {
        ...
        recommendations.push(
          'Soil pH is slightly outside optimal range (6.0-7.5). Consider adjusting with lime or sulfur.',
        );
      } else {
        recommendations.push(
          'Soil pH is critically imbalanced. Urgent soil treatment recommended.',
        );
      }
    } else {
      soilScore += 5; // Neutral if unknown
      recommendations.push(
        'Soil pH data missing. Testing is highly recommended.',
      );
    }
    ...
    if (n > 40) soilScore += 7;
      ...
      recommendations.push(
        'Low Nitrogen detected. Consider Nitrogen-rich fertilizers.',
      );
    ...
    if (p > 30) soilScore += 7;
      ...
      recommendations.push(
        'Low Phosphorus detected. Check root health and consider phosphate application.',
      );
    ...
    if (k > 150) soilScore += 6;
      ...
      recommendations.push(
        'Potassium levels are low. This may affect crop resistance.',
      );
    ...
    if (parcel.harvests.length > 0) {
      const avgYield =
        parcel.harvests.reduce((acc, h) => acc + (h.yieldPerHectare ?? 0), 0) /
        parcel.harvests.length;
      if (avgYield >= 5.0) yieldScore = 30;
      else if (avgYield >= 3.5) yieldScore = 20;
      else {
        yieldScore = 10;
        recommendations.push(
          'Historical yield is below regional averages. Review crop suitablity.',
        );
      }
    }
    ...
    if (recentPests > 1) {
      recommendations.push(
        `High frequency of pests (${recentPests} incidents) in last 6 months. Review pest management plan.`,
      );
    }
    ...
    if (freq.includes('daily') || freq.includes('auto')) irrigationScore = 20;
      ...
      recommendations.push(
        'Irrigation frequency is insufficient or undocumented.',
      );

    const totalScore = Math.round(
      soilScore + yieldScore + pestScore + irrigationScore,
    );
