...
const FORBIDDEN_STATUSES = ['FORBIDDEN', 'FORBIDDEN_ZONES', 'NOT_APPLICABLE', 'UNDER_ERADICATION'];
...
export class RegionalVaccineService {
    ...
    async generateSmartPlan(animalId: string) {
        ...
        if (!field) throw new BadRequestException('Aucune exploitation associée à cet animal');
        ...
        const resolvedField = await this.prisma.field.findUnique({ where: { id: field.id } });
        if (!resolvedField?.countryCode) {
            throw new BadRequestException('Impossible de déterminer le pays depuis les coordonnées GPS');
        }
        ...
        let validRegulations = regulations.filter(r => !FORBIDDEN_STATUSES.includes(r.status));

        // 6. Cas spécial Argentine — FA selon la zone
        if (resolvedField.countryCode === 'AR' && resolvedField.regionCode === 'AR-PATAGONIA') {
            validRegulations = validRegulations.filter(r => r.vaccine.code !== 'FMD');
        }
        ...
        for (const reg of validRegulations) {
            ...
            if (lastDate) {
                scheduledDate = new Date(lastDate.getTime() + reg.intervalDays * 86400000);
            } else {
            ...
            const schedule = await this.prisma.vaccineSchedule.create({
                data: {
                    ...
                    vaccineId: reg.vaccineId,
                    scheduledDate,
                    isMandatory: ['MANDATORY', 'MANDATORY_ZONES', 'MANDATORY_CONDITIONAL'].includes(reg.status),
    ...
    async checkForbidden(vaccineCode: string, fieldId: string, species: string): Promise<boolean> {
        const field = await this.prisma.field.findUnique({ where: { id: fieldId } });
        if (!field?.countryCode) return false;

        const country = await this.prisma.country.findUnique({ where: { code: field.countryCode } });
        if (!country) return false;

        const vaccine = await this.prisma.vaccine.findUnique({ where: { code: vaccineCode } });
