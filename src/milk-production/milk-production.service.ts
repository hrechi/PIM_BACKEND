...
export class MilkProductionService {
  ...
  async create(data: any, farmerId: string) {
    ...
    if (!animal) {
      throw new NotFoundException(
        `Animal with ID ${animalId} not found or access denied`,
      );
    }

    if (animal.animalType.toLowerCase() !== 'cow') {
      throw new BadRequestException(
        'Only cows can have milk production records',
      );
    }
  ...
  async remove(id: string, farmerId: string) {
    ...
    if (!record) {
      throw new NotFoundException(
        `Milk record with ID ${id} not found or access denied`,
      );
    }
  ...
  async update(id: string, data: any, farmerId: string) {
    ...
    const morning =
      morningL !== undefined
        ? parseFloat(morningL || 0)
        : Number(record.morningL);
    const evening =
      eveningL !== undefined
        ? parseFloat(eveningL || 0)
        : Number(record.eveningL);
    const total = morning + evening;
  ...
  async getAnalytics(
    ...
    const effectiveMonth =
      selectedMonth !== undefined ? selectedMonth - 1 : now.getMonth();
    ...
    if (timeframe === 'year') {
      ...
      rawMonthlyTrend.forEach((row) => {
        ...
        monthlySum.set(
          month,
          (monthlySum.get(month) || 0) + Number(row.totalL),
        );
      });
    ...
    const herdDetails = await Promise.all(
      herdStats.map(async (stat) => {
