...
import { RotationAnalysisDto, RecommendedCropDto, AvoidCropDto } from './dto/rotation-analysis.dto';
...
export class CropRotationService {
  ...
  async getCropRotationPlan(parcelId: string): Promise<RotationAnalysisDto> {
    ...
    const { recommendedCrops, cropsToAvoid } = await this.calculateRotationRules(parcel);
    const sustainabilityScore = this.calculateSustainabilityScore(parcel, cropsToAvoid.length);

    let explanation = 'Based on current soil metrics and historical crop data, the recommended crops will optimize yield while resting the soil from depleted nutrients.';
    
    // AI Enhancement using existing GROQ_API_KEY
    try {
      const groqExplanation = await this.generateAILogicExplanation(parcel, recommendedCrops, cropsToAvoid, sustainabilityScore);
      if (groqExplanation) {
  ...
  private async calculateRotationRules(parcel: any): Promise<{ recommendedCrops: RecommendedCropDto[], cropsToAvoid: AvoidCropDto[] }> {
    ...
    if (recentCrops.length >= 2 && recentCrops[0] === recentCrops[1]) {
      avoid.push({
        ...
        reason: 'Recently planted twice in a row. Planting again will diminish soil nutrients and increase disease risk.',
      });
    }

    // AVOID logic for recent diseases
    const recentDiseases = parcel.pests.map(p => (p.issueType || '').toLowerCase());
    if (recentDiseases.some(d => d.includes('fungus') || d.includes('blight'))) {
      avoid.push({
        name: 'Tomatoes',
        reason: 'Parcel has a history of blight/fungus. Tomatoes are highly susceptible.',
      });
      avoid.push({
        name: 'Potatoes',
    ...
    if (n < 40) { // e.g. low nitrogen
      recommended.push({ name: 'Beans', reason: 'Legumes fix nitrogen in the soil, restoring depleted levels.' });
      recommended.push({ name: 'Peas', reason: 'Low nitrogen demand and acts as a natural soil restorative.' });
    } else if (n > 80 && p > 50) {
      recommended.push({ name: 'Corn', reason: 'High soil nutrients are optimal for heavy feeders like corn.' });
      recommended.push({ name: 'Wheat', reason: 'Rich soil supports dense cereal crops.' });
    }

    if (parcel.soilPh !== null) {
      if (parcel.soilPh < 6.0) {
        recommended.push({ name: 'Oats', reason: 'More tolerant of acidic soils.' });
      } else if (parcel.soilPh > 7.5) {
        recommended.push({ name: 'Barley', reason: 'Handles alkaline conditions well.' });
      }
    }

    // Fallback if no specific condition matched
    if (recommended.length === 0) {
      recommended.push({ name: 'Soybeans', reason: 'Excellent rotational crop that improves soil structure.' });
      recommended.push({ name: 'Radish', reason: 'Deep roots help to break up compacted soil.' });
    }
  ...
  private calculateSustainabilityScore(parcel: any, avoidanceCount: number): number {
    let score = 100;

    // Repetition penalty
    const recentCrops = parcel.crops.map(c => c.cropName);
    if (recentCrops.length >= 2 && recentCrops[0] === recentCrops[1]) score -= 20;
  ...
  private async generateAILogicExplanation(
    ...
    if (!apiKey) {
      this.logger.warn('GROQ_API_KEY is not set. AI enhancement will be disabled.');
      return null;
    }
