...
export class ExpensesService {
    ...
    async create(userId: string, dto: CreateExpenseDto) {
        const { fieldId, animalId, amount, category, date, notes, receiptUrl } = dto;
        ...
        return this.prisma.expense.create({
            ...
            include: {
                animal: { select: { id: true, name: true, nodeId: true, tagNumber: true } },
            },
        });
    }
    ...
    async findAll(userId: string, query: {
        ...
        const { fieldId, category, startDate, endDate, animalId, limit = 50, offset = 0 } = query;
    ...
    async update(id: string, userId: string, dto: UpdateExpenseDto) {
        ...
        return this.prisma.expense.update({
            ...
            data: {
                ...
                animal: animalId ? { connect: { id: animalId } } : (animalId === null ? { disconnect: true } : undefined),
            },
        });
    }
    ...
    async getAnimalSummary(animalId: string, userId: string) {
        const animal = await this.prisma.animal.findFirst({
            ...
            include: { medicalEvents: true, expenses: true, childrenFromMother: true }
        });

        if (!animal) throw new NotFoundException('Animal not found');

        let totalCost = 0;
        const entryCost = animal.origin === 'born' ? (Number(animal.birthCost) || 0) : (Number(animal.purchasePrice) || 0);
        totalCost += entryCost;

        const breakdown = { feed: 0, vet: 0, meds: 0, equip: 0, labor: 0, other: 0 };
        ...
        animal.expenses.forEach(exp => {
            ...
            else if (cat.includes('vet') || cat.includes('med')) breakdown.meds += amt;
        ...
        const monthlyTrend = Array.from(monthlyMap.entries()).map(([month, amount]) => ({ month, amount })).sort((a, b) => a.month.localeCompare(b.month));

        const salePrice = Number(animal.salePrice) || 0;
        const margin = animal.status === 'sold' ? salePrice - totalCost : 0;
        const marginPercent = totalCost > 0 && animal.status === 'sold' ? (margin / totalCost) * 100 : 0;

        const startD = animal.origin === 'born' ? animal.lastBirthDate || animal.createdAt : animal.purchaseDate || animal.createdAt;
        const endD = animal.status === 'sold' && animal.saleDate ? animal.saleDate : new Date();
        const durationDays = Math.max(1, Math.floor((endD.getTime() - startD.getTime()) / (1000 * 3600 * 24)));
