...
export class VaccinesService {
  ...
  async getCountryRegulations(
    ...
    return this.prisma.vaccineRegulation.findMany({
      where: {
        ...
        ...(regionCode
          ? {
              OR: [{ regionId: null }, ...(regionId ? [{ regionId }] : [])],
            }
          : {}),
      },
  ...
  async getFieldRegulations(fieldId: string, species?: string) {
    ...
    if (!countryCode) {
      throw new BadRequestException(
        'Impossible de déterminer le pays depuis les coordonnées GPS du champ',
      );
    }
  ...
  async createRecord(dto: CreateVaccineRecordDto) {
    ...
    if (!vaccine)
      throw new NotFoundException(`Vaccin ${dto.vaccineCode} inconnu`);
    ...
    for (const animalId of dto.animalIds) {
      ...
      const record = await this.prisma.vaccineRecord.create({
        data: {
          ...
          lotNumber: dto.lotNumber ?? null,
          nextDueDate:
            schedule?.isRecurring && schedule?.recurrenceDays
              ? new Date(
                  administeredAt.getTime() + schedule.recurrenceDays * 86400000,
                )
              : null,
        },
      });
  ...
  async markDone(
    scheduleId: string,
    body: { administeredBy: string; doseGiven: number; lotNumber?: string },
  ) {
    ...
    if (schedule.isRecurring && schedule.recurrenceDays) {
      const nextDate = new Date(
        now.getTime() + schedule.recurrenceDays * 86400000,
      );
      await this.prisma.vaccineSchedule.create({
        data: {
          animalId: schedule.animalId,
