...
export class VaccinesController {
  ...
  getRegulations(
    ...
    return this.vaccinesService.getCountryRegulations(
      code,
      species,
      regionCode,
    );
  ...
  getUpcoming(@Query('fieldId') fieldId: string, @Query('days') days?: string) {
    return this.vaccinesService.getUpcoming(
      fieldId,
  ...
  async testNotification(@Request() req: any) {
    ...
    if (!user?.fcmToken) {
      return {
        ok: false,
        message:
          'No FCM token found. Make sure the app sent the token after login.',
      };
    }
